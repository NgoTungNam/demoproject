...
app.post("/api/products", upload.single("image"), (req, res) => {
  ...
  db.query(
    sql,
    [name, price, description, image_url, category_id || 1],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
...
app.post("/api/orders", (req, res) => {
  ...
  db.query("SELECT id FROM users WHERE id = ?", [user_id], (uErr, uResults) => {
    ...
    if (uErr || uResults.length === 0) {
      console.warn(
        `[Warning] User ID ${user_id} không tồn tại. Đang đặt là NULL.`,
      );
      finalUserId = null;
    }
...
app.get("/api/payment/vnpay/return", (req, res) => {
  ...
  if (isValid && responseCode === "00") {
    ...
    res.redirect(
      `${frontendUrl}/checkout/payment-result?status=success&method=vnpay&orderId=${orderId}`,
    );
  } else {
    res.redirect(
      `${frontendUrl}/checkout/payment-result?status=failed&method=vnpay&orderId=${orderId}`,
    );
  }
});
